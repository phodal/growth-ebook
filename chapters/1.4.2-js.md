...
###javascript变量与内存
 ...
 javascript变量 类型分为原始类型和引用类型
 
  - 原始类型(基本类型):undefined、Null、Number、String、boolean
  - 引用类型:Object、function、Array  
    原始类型的变量存放在栈内存。  
    引用类型的变量 地址(指针)存放在栈内存,具体内容存放在堆内存中。   
    原始类型的比较是值得比较，引用类型的比较是引用的比较   
``` javascript
var p1 = '{}';
var p2 = '{}';
...
var p1 = {};
var p2 = {};
console.log(p1==p2) //false 因为p1和p2在堆内存中的地址是不同的
```  